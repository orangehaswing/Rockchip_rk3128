...
static void pinctrl_rk3128_pwm_config(struct rk3128_grf *grf, int pwm_id)
{
	switch (pwm_id) {
	case PERIPH_ID_PWM0:
...
static void pinctrl_rk3128_spi_config(struct rk3128_grf *grf, int cs)
{
	switch (cs) {
	case 0:
...
static void pinctrl_rk3128_uart_config(struct rk3128_grf *grf, int uart_id)
{
	switch (uart_id) {
...
static int rk3128_pinctrl_request(struct udevice *dev, int func, int flags)
{
	...
	case PERIPH_ID_PWM2:
	case PERIPH_ID_PWM3:
		pinctrl_rk3128_pwm_config(priv->grf, func);
		break;
	case PERIPH_ID_I2C0:
	case PERIPH_ID_I2C1:
	case PERIPH_ID_I2C2:
		...
		break;
	case PERIPH_ID_SPI0:
		pinctrl_rk3128_spi_config(priv->grf, flags);
		break;
	case PERIPH_ID_UART0:
	case PERIPH_ID_UART1:
	case PERIPH_ID_UART2:
		pinctrl_rk3128_uart_config(priv->grf, func);
		break;
