...
/* 396Mhz ddr timing */
const struct rk3128_ddr_timing ddr_timing = {0x18c,
...
static void rkdclk_init(struct rk3128_sdram_priv *priv)
{
	struct rk3128_pll *pll = &priv->cru->pll[1];
...
void phy_pctrl_reset(struct rk3128_sdram_priv *priv)
{
	struct rk3128_ddr_phy *ddr_phy = priv->phy;
...
void phy_dll_bypass_set(struct rk3128_sdram_priv *priv, unsigned int freq)
{
	struct rk3128_ddr_phy *ddr_phy = priv->phy;
...
static void memory_init(struct rk3128_sdram_priv *priv)
{
	struct rk3128_ddr_pctl *pctl = priv->pctl;
...
static void data_training(struct rk3128_sdram_priv *priv)
{
	struct rk3128_ddr_phy *ddr_phy = priv->phy;
	struct rk3128_ddr_pctl *pctl = priv->pctl;
	u32 value;
...
static void move_to_config_state(struct rk3128_sdram_priv *priv)
{
	unsigned int state;
	struct rk3128_ddr_pctl *pctl = priv->pctl;
...
static void move_to_access_state(struct rk3128_sdram_priv *priv)
{
	unsigned int state;
	struct rk3128_ddr_pctl *pctl = priv->pctl;
...
static void pctl_cfg(struct rk3128_sdram_priv *priv)
{
	struct rk3128_ddr_pctl *pctl = priv->pctl;
	u32 burst_len;
	u32 reg;
...
static void phy_cfg(struct rk3128_sdram_priv *priv)
{
	struct rk3128_ddr_phy *ddr_phy = priv->phy;
	struct rk3128_service_sys *axi_bus = priv->axi_bus;
...
void dram_cfg_rbc(struct rk3128_sdram_priv *priv)
{
	char noc_config;
	int i = 0;
	struct rk3128_ddr_config config = priv->ddr_config;
	struct rk3128_service_sys *axi_bus = priv->axi_bus;
...
static void sdram_all_config(struct rk3128_sdram_priv *priv)
{
	u32 os_reg = 0;
	u32 cs1_row = 0;
	struct rk3128_ddr_config config = priv->ddr_config;
...
size_t sdram_size(void)
{
	...
	struct rk3128_grf *grf = (void *)GRF_BASE;
	...
	/* row + col + bank + bw(rk3128 only support 16bit, so fix in 1) */
...
{
	struct rk3128_sdram_priv sdram_priv;
